...
if [ -e ./canu_assembly.$jobid.meryl/merylIndex ] ; then
...
if [ -e canu_assembly.$jobid.meryl.tar.gz ]; then
  exist1=true
else
  exist1=false
fi
...
/sw/bioinfo/canu/2.2/rackham/bin/meryl k=22 threads=4 memory=6 \
  count \
    segment=$jobid/01 ../../canu_assembly.seqStore \
    output ./canu_assembly.$jobid.meryl.WORKING \
&& \
mv -f ./canu_assembly.$jobid.meryl.WORKING ./canu_assembly.$jobid.meryl
