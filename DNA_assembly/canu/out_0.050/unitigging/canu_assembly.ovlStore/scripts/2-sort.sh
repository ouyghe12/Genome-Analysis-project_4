...
#  This script should be executed from unitigging/canu_assembly.ovlStore.BUILDING/, but the binary needs
...
if [ -e ./canu_assembly.ovlStore.BUILDING/$jobname.info -a ! -e ./canu_assembly.ovlStore.BUILDING/$jobname.started ] ; then
  echo "Sorting job finished; info file './canu_assembly.ovlStore.BUILDING/$jobname.info' exists."
  exit
fi
#
#  Sort!
...
$bin/ovStoreSorter \
  ...
  -C  ./canu_assembly.ovlStore.config \
  -f \
  -s $jobid \
  -M 3 
